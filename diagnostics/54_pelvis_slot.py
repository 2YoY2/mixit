#!/usr/bin/env python3
"""Probe 54: which slot explains the PELVIS movement best?
...
def main():
    ...
    for scene in (1, 4):
        ...
        for g in groups:
            ...
            Y = root[:nw][m_]
            spdY = np.linalg.norm(np.diff(root[:nw], axis=0), axis=1)
        ...
        print(f"\n=== SCENE {scene} (N={len(best1)}, "
              f"{(time.time()-t0)/60:.1f}min)  CV-R2 of pelvis:", flush=True)
    ...
    print("probe 54 done", flush=True)
